...
sample_size1 <- data_locomotion_reduced %>%
  select(Individual_ID, Treatment, Sex) %>%
  unique() %>%
...
data_ymaze_reduced_long2 <- data_ymaze_reduced_long %>%
  select(reps, alter, partial, Individual_ID, Sex, Treatment) %>%
  unique()


#males
# plot2.3 <- data_ymaze_reduced_long2 %>%
#   filter(Sex == "M") %>%
...
plot2.12 <- na.omit(data_ymaze_reduced_long2) %>%
  # filter(Type == "alter") %>%
...
plot2.13 <- na.omit(data_ymaze_reduced_long2) %>%
  # filter(Type == "reps") %>%
...
sample_size3 <- data_ymaze_reduced_long2 %>%
  group_by (Treatment, Sex) %>%
  summarise(n = n())
...
sample_size4 <- data_ymaze_reduced_long2 %>%
  group_by (Treatment, Sex) %>%
  summarise(n = n())
...
#TODO not sure if this is correct since we have repetitions of data
...
m5 <-glmer(partial~ Sex*Treatment + (1|Individual_ID) + (1|Batch), family = poisson, data = data_ymaze_reduced_long)
Anova(m5)
...
sample_size2 <- data_startle_binary_reduced %>%
  group_by (Treatment, Sex) %>%
  summarise(n = n())
...
#TODO change order of correlations on x axis to match order of reporting in Table 2
plot <- summary %>%
...
GS_summary$assay <- c("loco~startle", "startle~reps", "loco~reps")
...
GM_summary$assay <- c("loco~startle", "startle~reps", "loco~reps")
...
I_summary$assay <- c("loco~startle", "startle~reps", "loco~reps")
...
```{r,}
plots <- plot / (plot1 + plot2) + plot_annotation(tag_levels = 'A')
