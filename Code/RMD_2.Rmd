...
data_ymaze <- read.csv(here("Data/data_ymaze.csv"))
#TODO why is there data in the 15th ymaze?

data_startle_act <- read.csv(here("Data/data_startle_act.csv")) #no startles removed
#TODO need to add column of individual ID to session data to join
...
```{r removing individuals, eval = FALSE}
...
plot1.4 <- na.omit(data_locomotion_reduced) %>% 
  ...
  geom_violin(position = position_dodge(1)) + stat_summary(fun= mean, geom = "point", position = position_dodge(1)) + labs(x = "Treatment", y = "log(Arena distance)")
...
```{r 34_summary_histogram}

plot2.4 <- data_ymaze_reduced_long %>% pivot_longer(cols = c('reps', 'alter'), names_to = "Type", values_to = "Count_type") %>%
...
plot2.1a <- data_ymaze_reduced_long %>%
  filter(Sex == "F") %>%
  ggplot(aes(x = Trigram, y = Count, fill = Treatment)) +
  geom_bar(stat = "sum") +
  ylim(0,7000) +
...
plot3.2 <- ggplot(data=data_startle_act_reduced, aes(x = startle_number, y = distance)) +
