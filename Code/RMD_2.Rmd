...
plot2.2 <- data_ymaze_reduced_long_trigramtype %>%
...
plot2.3 <- ((plot2.1 + plot2.2) + plot_annotation(tag_levels = 'A'))
plot2.3 

```

Treatment and Sex effects all trigrams
...
data_ymaze_reduced_long_trigramtype$Type<-as.factor(data_ymaze_reduced_long_trigramtype$Type)
data_ymaze_reduced_long_trigramtype$Sex<-as.factor(data_ymaze_reduced_long_trigramtype$Sex)


#TODO I don't think it makes sense to plot it like this since we don't test for a trigram type interaction. It makes more sense to plot reps vs alter separately

# #males
# plot2.6 <- na.omit(data_ymaze_reduced_long_trigramtype) %>% 
#  filter(Sex == "M") %>%
...
# 
# #females
# plot2.7 <- na.omit(data_ymaze_reduced_long_trigramtype) %>% 
#  filter(Sex == "F") %>%
#   ggplot(aes(x = Treatment, y = Count_type, fill = Type)) + 
#   geom_violin(position = position_dodge(1)) + stat_summary(fun.data=data_summary, position = position_dodge(1))  + labs(x = "Treatment", y = "Count_type") +
#   ylim(0,200) +
...
# 
# 
# data_ymaze_reduced_long_trigramtype2 <- data_ymaze_reduced_long_trigramtype [c("Individual_ID", "Sex", "Type","Treatment", "Count_type")]
# 
# plot2.9 <- na.omit(data_ymaze_reduced_long_trigramtype) %>%
#   filter(Sex == "M") %>%
#   ggplot(aes(x = Treatment, y = Count_type, fill = Type)) + 
...
# 
# 
# plot2.10 <- na.omit(data_ymaze_reduced_long_trigramtype) %>%
#   filter(Sex == "F") %>%
#   ggplot(aes(x = Treatment, y = Count_type, fill = Type)) + 
#   geom_bar(position = "fill", stat = "identity")+
#   theme_classic() + theme(text = element_text(size = 15)) +
...
#plotting reps and alters separately

# plot2.12 <- na.omit(data_ymaze_reduced_long4) %>%
#   filter(Type == "alter") %>%
#   ggplot(aes(x = Treatment, y = Count_type, fill = Sex)) + 
#   geom_bar(position = "fill", stat = "identity")+
#   theme_classic() + theme(text = element_text(size = 15), legend.position = "none") +
...
plot2.12 <- na.omit(data_ymaze_reduced_long4) %>%
  filter(Type == "alter") %>%
  ggplot(aes(x = Treatment, y = Count_type, fill = Sex)) + geom_col(position = "stack")+
  theme_classic() + theme(text = element_text(size = 15), legend.position = "none") +
  labs(x = "Treatment", y = 'Count', title = 'Alternations') +
  scale_fill_brewer(palette = "Set1") +
  ylim(0,10000)
...
# plot2.13 <- na.omit(data_ymaze_reduced_long4) %>%
#   filter(Type == "reps") %>%
#   ggplot(aes(x = Treatment, y = Count_type, fill = Sex)) +
#   geom_bar(position = "fill", stat = "identity") +
#   theme_classic() + theme(text = element_text(size = 15), legend.position = "none") +
...
plot2.14 <- (plot2.12 + plot2.13) 
plot2.14 # red = female, blue = male
```
...
hist(data_startle_act_reduced$distance)
data_startle_act_reduced$log_distance <-log(data_startle_act_reduced$distance+1)

hist(data_startle_act_reduced$log_distance)
...
plot3.1 <- na.omit(g) %>%
  ...
  ylim(-0.2, 2.5) + theme(text = element_text(size = 15), legend.position = "none") + labs(x = 'Treatment',
...
summary(m) #model included in results
...
m <- glmer(Count_type~ Type*Sex*Treatment + (1|Individual_ID) + (1|Batch), family = poisson, data = data_ymaze_reduced_long_trigramtype)
Anova(m)

ma <- glmer(Count_type~ Type + (1|Individual_ID) + (1|Batch), family = poisson, data = data_ymaze_reduced_long_trigramtype)
Anova(ma)
summary(ma)
...
#TODO asymmetry: 0 = random left vs right (symmetrical), positive = excess of left turns, negative = excess of right turns
...
#individual_ID
1.57 / (1.57 + 0.053 + 0.098 + 0.558)*100

#batch
0.053 / (1.57 + 0.053 + 0.098 + 0.558)*100
...
#individual_ID
1.936 / (1.936 + 0.106 + 0.025 + 1.156)*100


0.106 / (1.936 + 0.106 + 0.025 + 1.156)*100
...
m5 <- lmer(log(distance +1) ~ Sex*Treatment + (1|Batch) + (startle_number|Individual_ID),
...
m6 <- lmer(log(distance +1) ~ Sex*Treatment + Sex*startle_number + (1|Batch) + (1|Individual_ID), data = data_startle_act_reduced)
...
m6b <- lmer(log(distance +1) ~ Sex*Treatment + Sex*startle_number + Treatment*startle_number + (1|Batch) + (1|Individual_ID), data = data_startle_act_reduced)
...
m6c <- lmer(log(distance +1) ~ Sex*Treatment*startle_number + (1|Batch) + (1|Individual_ID), data = data_startle_act_reduced)

Anova(m6c) #model used in manuscript
...
```{r}
#Used to see if there is a difference between treatments in if the flies moved or not
...
m6d <- glmer(startle_binary~Sex*Treatment + (1|Batch), family = binomial, data = data_startle_binary_reduced) #far more isolated flies were startled
Anova(m6d)
summary(m6d)

#viewing contrast beteween I and GS

m6e <-glmer(startle_binary~ Sex*relevel(factor(Treatment), ref = "GS") + (1|Batch), family = binomial, data = data_startle_binary_reduced)
summary(m6e)
...
#could do a simple t test between the different treatments since we get the correlation and SE. Just look at between treatment and between sex. 
...
## Example analysis of correlation heterogeneity

### Locomotion & Y-maze data

```{r}
library(MCMCglmm)

glimpse(data_loco_ymaze)

# simple model with only corss-trait covariances
# note - to get correlations Cov_1,2/sqrt(Var_1 * Var_2)
model_cor1 <- MCMCglmm(cbind(reps, alter, avg_loco) ~ -1 + trait + trait:Sex.x + trait:Treatment.x,
                       rcov = ~us(trait):units,
                       family = c("gaussian", "gaussian", "gaussian"),
                       nitt = 100000, burnin = 20000, thin = 80,
                       data = as.data.frame(data_loco_ymaze),
                       prior = list(R = list(V = diag(3), nu = 2.002)))

plot(model_cor1$Sol)
plot(model_cor1$VCV)
autocorr.plot(model_cor1$VCV)
summary(model_cor1)
# quick way to get posteriors of correlations instead of covariances
summary(posterior.cor(model_cor1$VCV))
HPDinterval(posterior.cor(model_cor1$VCV))
colMeans(posterior.cor(model_cor1$VCV))

# block-diagonal model with correlations varying between exp groups
model_cor2 <- MCMCglmm(cbind(reps, alter, avg_loco) ~ -1 + trait + trait:Sex.x + trait:Treatment.x,
...
## Correlation analysis for manuscipt
...
### Model across treatment and sex

```{r}

#TODO priors need to be changes so model outputs will be altered

model_cor1 <- MCMCglmm(cbind(avg_loco, startle_binary, reps) ~ -1 + trait + trait:Sex.x + trait:Treatment.x,
                       rcov = ~us(trait):units,
                       family = c("gaussian", "categorical", "poisson"),
                       nitt = 100000, burnin = 20000, thin = 80,
...
summary$mean <- means
summary<-head(summary, 3)
summary$assay <- c("loco~startle", "loco~reps", "startle~reps")
...
plot <- summary %>%
  ...
  geom_point(position = position_dodge(1), aes(size = 2)) + theme_classic() +
  ylim(-1, 1) + labs(x = 'Assay correlations',
                         y = 'Posterior mean',
                         title = "Overall model across treatment and sex") +
...
model_cor2 <- MCMCglmm(cbind(avg_loco, startle_binary, reps) ~ -1 + trait + trait:Sex.x + trait:Treatment.x,
                       rcov = ~us(trait:at.level(Treatment.x, "GS")):units +
                         us(trait:at.level(Treatment.x, "GM")):units + 
                         us(trait:at.level(Treatment.x, "I")):units,
                       family = c("gaussian", "categorical", "poisson"),
...
GS_summary<-head(GS_summary, 3)
GS_summary$assay <- c("loco~startle", "loco~reps", "startle~reps")
...
GM_summary<-head(GM_summary, 3)
GM_summary$assay <- c("loco~startle", "loco~reps", "startle~reps")
GM_summary$treatment <- c("GM", "GM", "GM")

#I summary
I_means <- colMeans(posterior.cor(model_cor2$VCV[,10:18]))
I_summary <- as.data.frame(HPDinterval(posterior.cor(model_cor2$VCV[,10:18])))
I_summary$mean <- I_means
I_summary<-head(GM_summary, 3)
I_summary$assay <- c("loco~startle", "loco~reps", "startle~reps")
...
plot1 <- treatment_merged %>%
  ...
  ylim(-1,1) + labs(x = 'Assay correlations',
                         y = 'Posterior mean',
                         col = 'Treatment',
                         title = "Correlations based on treatment",
...
M_summary<-head(M_summary, 3)
M_summary$assay <- c("loco~startle", "loco~reps", "startle~reps")
...
F_summary<-head(F_summary, 3)
F_summary$assay <- c("loco~startle", "loco~reps", "startle~reps")
...
plot2 <- sex_merged %>%
  ...
  ylim(-1,1) + labs(x = 'Assay correlations',
                         y = 'Posterior mean',
                         col = 'Treatment',
                         title = "Correlations based on sex",
                         legend_position = "none") +
  theme(
    legend.position = "none",
    axis.title.x = element_text(size = 14),
